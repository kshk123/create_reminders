...
This feature requires a local Python bridge (distributed separately from the AMO add-on) to communicate with Apple Reminders.
...
1) Download the **Apple bridge bundle** from the GitHub release/download (contains `apple_reminders_bridge.py`, `start-bridge.sh`, and the launch agent files).  
2) Start it locally:
```bash
